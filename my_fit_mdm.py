...
# remove results.npySome
paths = [path for path in paths if "results.npy" not in path]
    ...
    for idx in range(num_seqs):
        ...
        if False:
            mesh_p = trimesh.Trimesh(
                vertices=keypoints_3d[idx].unsqueeze(0).detach().cpu().numpy().squeeze(), process=False)
            mesh_p.export(dir_save + "/" + "%04d" % idx + "_gt.ply")

        mesh_p = trimesh.Trimesh(
            vertices=outputp.vertices[idx].detach().cpu().numpy().squeeze(),
        ...
        # save the pkl
        param = {}
        param["beta"] = new_opt_betas[idx].unsqueeze(0).detach().cpu().numpy()
        param["pose"] = new_opt_pose[idx].unsqueeze(0).detach().cpu().numpy()
        param["cam"] = new_opt_cam_t[idx].unsqueeze(0).detach().cpu().numpy()
